...
class VideoRecordingWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        if self.video_dir is not None:
            ...
            self.file_path = self.video_dir / f"{ts}.mp4"
        return result
    ...
    def close(self):
        # 追加：保证只跑 1 个 episode 时，最后一个视频也会被改成 *_success{0/1}.mp4
        self.video_recorder.stop()
        if self.video_dir is not None and self.file_path is not None and self.file_path.exists():
            if "_success" not in self.file_path.stem:
                new_filestem = f"{self.file_path.stem}_success{int(self.is_success)}"
                new_file_path = self.video_dir / f"{new_filestem}.mp4"
                os.rename(self.file_path, new_file_path)
