...
class Optimizer:
    ...
    def retarget(self, ref_value, fixed_qpos, last_qpos):
        ...
        # print(f"ref_value_shape: {ref_value.shape}, fixed_qpos_shape: {fixed_qpos.shape}, last_qpos_shape: {last_qpos.shape}")
...
class PositionOptimizer(Optimizer):
    ...
    def set_retarget_weight(self, wrist_enhance_weight: float = 3):
        self.weighted_l1_loss = lambda x, y: torch.mean(torch.norm(x - y, p=1, dim=-1) * torch.tensor([1, 1, 1, 1, 1, wrist_enhance_weight], dtype=torch.float32))
    
    def set_retarget_gripper_weight(self, wrist_enhance_weight: float = 3):
        self.weighted_l1_loss = lambda x, y: torch.mean(torch.norm(x - y, p=1, dim=-1) * torch.tensor([1, 1,  wrist_enhance_weight], dtype=torch.float32))
    def __init__(
        self,
        robot: RobotWrapper,
        target_joint_names: List[str],
    ...
    def get_objective_function(
        ...
        pair_self_diff = lambda x: x.unsqueeze(1) - x.unsqueeze(0)
        ...
        def objective(x: np.ndarray, grad: np.ndarray) -> float:
            ...
            # dis_loss = self.huber_loss(torch_body_pos, torch_target_pos)
            dis_loss = self.weighted_l1_loss(torch_body_pos, torch_target_pos)
            vec_loss = 0 #self.l1_loss(pair_self_diff(torch_body_pos), pair_self_diff(torch_target_pos))
            # print(f"dis_loss: {dis_loss}, vec_loss: {vec_loss}")
            loss = dis_loss + vec_loss
