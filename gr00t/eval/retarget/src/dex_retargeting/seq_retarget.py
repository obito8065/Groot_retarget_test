...
class SeqRetargeting:
    ...
    def warm_start(
        ...
        warmup_qpos6d = np.concatenate([
            target_root_pose[:3, 3],  # position
            rotations.euler_from_matrix(target_root_pose[:3, :3], 0, 1, 2, extrinsic=False)  # rotation, euler
        ])
        ...
        for num, joint_name in enumerate(self.optimizer.target_joint_names):
            if joint_name in name_list:
                ...
                self.last_qpos[num] = warmup_qpos6d[index]
