...
class RobotInferenceServer(BaseInferenceServer):
    ...
    def __init__(self, model, host: str = "*", port: int = 5555, api_token: str = None , use_msgpack: bool = False):
        super().__init__(host, port, api_token, use_msgpack)
    ...
    @staticmethod
    def start_server(policy: BasePolicy, port: int, api_token: str = None, use_msgpack: bool = False):
        server = RobotInferenceServer(policy, port=port, api_token=api_token, use_msgpack=use_msgpack)
        server.run()
