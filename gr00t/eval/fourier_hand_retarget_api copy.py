...
class FourierHandRetargetAPI:
    ...
    def _ensure_axisangle_continuity(self, current_rotvec: np.ndarray, side: str) -> np.ndarray:
        ...
        if quat_dot < 0:
            ...
            # 转回轴角
            corrected_rotvec = R.from_quat(current_quat).as_rotvec()
        else:
            corrected_rotvec = current_rotvec
        
        # 更新历史记录（存储修正后的四元数）
        self._last_quaternion[side] = current_quat.copy()
        
        return corrected_rotvec
    ...
    def reset(self, env_idx: Optional[int] = None):
        ...
        for side in self.hand_sides:
            ...
            self._last_quaternion[side] = None  # 重置轴角连续性历史
    ...
    def retarget_from_keypoints(
        ...
        注意:
            这个接口不包含wrist_rotvec信息，warmup效果可能不如retarget_from_45d
        """
        result = {}
        ...
        for side in self.hand_sides:
            ...
            # 如果是warmup阶段，但没有rotvec信息，使用单位旋转
            if self._episode_frame_count[side] < self.warm_up_steps:
                wrist_xyz = kp[0]
                wrist_quat_wxyz = np.array([1.0, 0.0, 0.0, 0.0])  # 单位四元数
            ...
            # 通过 SAPIEN FK 计算 hand_base_link 的实际位姿（与 hand_robot_viewer_fourier.py 保持一致）
            sapien_robot = self.sapien_robots[side]
            sapien_robot.set_qpos(qpos_full)
            hand_base_link_idx = self.hand_base_link_indices[side]
            link_pose = sapien_robot.get_links()[hand_base_link_idx].entity_pose
            ...
            wrist_pose = np.concatenate([wrist_pos, wrist_rotvec]).astype(np.float32)
            desired_indices = self.desired_joint_indices[side]
            finger_joints = qpos_full[desired_indices]  # [pinky, ring, middle, index, thumb_pitch, thumb_yaw]
            
            # 确保shape正确
            assert wrist_pose.shape == (6,), f"{side} wrist_pose shape错误: {wrist_pose.shape}"
            assert finger_joints.shape == (6,), f"{side} finger_joints shape错误: {finger_joints.shape}"
