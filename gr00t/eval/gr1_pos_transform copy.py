...
class BodyRetargeter:
    def __init__(self, urdf_path: Path, camera_intrinsics: Dict[str, float]):
        ...
        # 新增：保存上一帧的轴角（按env_idx分桶）
        self._last_axisangle = {}  # {env_idx: {'left': rotvec, 'right': rotvec}}
    ...
    def _ensure_axisangle_continuity(self, 
        ...
        if hand_label not in self._last_axisangle[env_idx]:
            self._last_axisangle[env_idx][hand_label] = current_axisangle.copy()
            return current_axisangle
        
        last_axisangle = self._last_axisangle[env_idx][hand_label]
        
        # 计算两个轴角向量的点积
        dot_product = np.dot(current_axisangle, last_axisangle)
        
        # 如果点积为负，说明两个轴角向量方向相反，这通常意味着等价表示的跳变
        # 此时将当前轴角取反，使其与上一帧保持一致的表示
        if dot_product < 0:
            corrected_axisangle = -current_axisangle
            print(f"  [{hand_label} hand, env={env_idx}] 检测到轴角跳变，已修正 (dot={dot_product:.3f})")
        else:
            corrected_axisangle = current_axisangle
        
        # 更新历史记录
        self._last_axisangle[env_idx][hand_label] = corrected_axisangle.copy()
        
        return corrected_axisangle

    # 修改 reset_ik_cache 方法，同时清空轴角缓存
    def reset_ik_cache(self, env_idx: Optional[int] = None):
        """
        清空 IK 历史缓存（last_solution）和轴角历史缓存。
        """
        if env_idx is None:
            self._ik_last_solution.clear()
            self._last_axisangle.clear()  # 新增
            return
        env_idx = int(env_idx)
        self._ik_last_solution.pop(env_idx, None)
        self._last_axisangle.pop(env_idx, None)  # 新增
