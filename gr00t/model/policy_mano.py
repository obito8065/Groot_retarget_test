                ...
                if (left_hand_orig is not None and right_hand_orig is not None and
                    ...
                    if not hasattr(self, '_observation_log_initialized'):
                        ...
                        with open(self._observation_log_file, 'w') as f:
                            ...
                            f.write("# 手指关节顺序（仿真格式）：[index, middle, ring, pinky, thumb_yaw, thumb_pitch]\n")
        ...
        if (self.use_eepose and self.use_fourier_hand_retarget and 
            ...
            if pred_left_keypoints_seq is not None and pred_right_keypoints_seq is not None:
                ...
                if D_kp == 21:
                    ...
                    # retarget后的旋转轴角上还相差一个mano到灵巧手到坐标系方向转换
                    # 定义坐标系转换矩阵
                    ...
                    # 对左手进行坐标系转换
                    # retarget_left_arm_seq[:, :, 3:] 是 (B, horizon, 3) 的轴角
                    B, T, _ = retarget_left_arm_seq.shape
                    left_rotvec = retarget_left_arm_seq[:, :, 3:].reshape(-1, 3)  # (B*T, 3)
                    
                    # 将轴角转换为旋转矩阵
                    left_rot = R.from_rotvec(left_rotvec)  # Rotation对象
                    left_rot_matrix = left_rot.as_matrix()  # (B*T, 3, 3)
                    
                    # 应用坐标系转换：rot_matrix_after = rot_matrix_robot @ OPERATOR2MANO_LEFT.T
                    left_rot_matrix_transformed = left_rot_matrix @ OPERATOR2MANO_LEFT.T  # (B*T, 3, 3)
                    
                    # 将旋转矩阵转回轴角
                    left_rot_transformed = R.from_matrix(left_rot_matrix_transformed)
                    left_rotvec_transformed = left_rot_transformed.as_rotvec()  # (B*T, 3)
                    
                    # 放回原位置
                    retarget_left_arm_seq[:, :, 3:] = left_rotvec_transformed.reshape(B, T, 3)
                    
                    # 对右手进行坐标系转换
                    right_rotvec = retarget_right_arm_seq[:, :, 3:].reshape(-1, 3)  # (B*T, 3)
                    
                    # 将轴角转换为旋转矩阵
                    right_rot = R.from_rotvec(right_rotvec)  # Rotation对象
                    right_rot_matrix = right_rot.as_matrix()  # (B*T, 3, 3)
                    
                    # 应用坐标系转换：rot_matrix_after = rot_matrix_robot @ OPERATOR2MANO_RIGHT.T
                    right_rot_matrix_transformed = right_rot_matrix @ OPERATOR2MANO_RIGHT.T  # (B*T, 3, 3)
                    
                    # 将旋转矩阵转回轴角
                    right_rot_transformed = R.from_matrix(right_rot_matrix_transformed)
                    right_rotvec_transformed = right_rot_transformed.as_rotvec()  # (B*T, 3)
                    
                    # 放回原位置
                    retarget_right_arm_seq[:, :, 3:] = right_rotvec_transformed.reshape(B, T, 3) 
                    ...
                    if not hasattr(self, '_retarget_log_initialized'):
                        ...
                        with open(self._retarget_log_file, 'w') as f:
                            ...
                            f.write("# L_finger_joint_names_6: L_pinky_proximal_joint L_ring_proximal_joint L_middle_proximal_joint L_index_proximal_joint L_thumb_proximal_pitch_joint L_thumb_proximal_yaw_joint\n")
                            f.write("# R_finger_joint_names_6: R_pinky_proximal_joint R_ring_proximal_joint R_middle_proximal_joint R_index_proximal_joint R_thumb_proximal_pitch_joint R_thumb_proximal_yaw_joint\n")
                            f.write("# 注意：finger joints顺序为6个主动关节 [pinky, ring, middle, index, thumb_pitch, thumb_yaw]，可直接用于仿真控制\n")
                        ...
                        try:
                            ...
                            with open(self._retarget_log_file, 'a') as f:
                                ...
                                for t in range(horizon):
                                    ...
                                    right_finger_joints = retarget_right_hand_seq[0, t, :] # (6,) = [pinky, ring, middle, index, thumb_pitch, thumb_yaw]
        ...
        if self.use_eepose:
            ...
            if not hasattr(self, '_robocasa_action_log_initialized'):
                ...
                with open(self._robocasa_action_log_file, 'w') as f:
                    ...
                    f.write("# L_finger_joint_names_6: L_pinky_proximal_joint L_ring_proximal_joint L_middle_proximal_joint L_index_proximal_joint L_thumb_proximal_pitch_joint L_thumb_proximal_yaw_joint\n")
                    f.write("# R_finger_joint_names_6: R_pinky_proximal_joint R_ring_proximal_joint R_middle_proximal_joint R_index_proximal_joint R_thumb_proximal_pitch_joint R_thumb_proximal_yaw_joint\n")
                    f.write("# 注意：这是保存unnormalized_action字典中的数据，用于对比验证\n")
                    f.write("# batch=0的数据，每个chunk的16个时间步(t=0~15)各记录一行\n")
                    f.write("#\n")
                ...
                try:
                    with open(self._robocasa_action_log_file, 'a') as f:
                        ...
                        for t in range(horizon):
                            ...
                            left_arm = unnormalized_action["action.left_arm"][0, t, :]      # (6,)
                            left_hand = unnormalized_action["action.left_hand"][0, t, :]    # (6,)
                            right_arm = unnormalized_action["action.right_arm"][0, t, :]    # (6,)
                            right_hand = unnormalized_action["action.right_hand"][0, t, :]  # (6,)
                            
                            # 拼接完整动作：[left_arm(6), left_hand(6), right_arm(6), right_hand(6)] = 24维
                            full_action = np.concatenate([
                                left_arm,     # L_wrist_xyz + L_rotvec
                                left_hand,    # L_finger_q1~6: [pinky, ring, middle, index, thumb_pitch, thumb_yaw]
                                right_arm,    # R_wrist_xyz + R_rotvec
                                right_hand    # R_finger_q1~6: [pinky, ring, middle, index, thumb_pitch, thumb_yaw]
                            ])
