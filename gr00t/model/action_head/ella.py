...
class PerceiverResampler(nn.Module):
    ...
    def forward(self, x: torch.Tensor, timestep_embedding: torch.Tensor = None):
        learnable_latents = self.latents.unsqueeze(dim=0).repeat(len(x), 1, 1) # [64, 768] -> [B, 64, 768]
        latents = learnable_latents + self.time_aware_linear(
            torch.nn.functional.silu(timestep_embedding) # timestep_embedding [B, 1, 768] -> latents:[B, 64, 768]
        )
        if self.input_dim is not None: # 2048
            x = self.proj_in(x)        # x [B, 10, 2048->768] x是encoded_features
        for p_block in self.perceiver_blocks:
            latents = p_block(x, latents, timestep_embedding=timestep_embedding) # latents: [B, 64, 768]
...
class ELLA(nn.Module):
    ...
    def forward(self, text_encode_features, timesteps):
        ...
        # [B, L, 2048], time_embedding [B, 1, 768] -> [B, 64, 768]
...
if __name__ == "__main__":
    ella = ELLA().cuda()
    text_encode_features = torch.randn((2, 10, 2048),device=torch.device("cuda"))
    timesteps = torch.randn((2),device=torch.device("cuda"))

    for i in range(10):
        outs = ella(text_encode_features, timesteps)