...
# LE_ROBOT_TASKS_FILENAME = "meta/new_tasks.jsonl"
LE_ROBOT_TASKS_FILENAME = "meta/tasks.jsonl"
print(f"#### LE_ROBOT_TASKS_FILENAME = {LE_ROBOT_TASKS_FILENAME} ####")
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        instr_use_episode_index: bool = True,
    ):
        """
        ...
        self.instr_use_episode_index = instr_use_episode_index
        self.enable_latent_embedding = None

    def update_configs(self, enable_latent_embedding):
        self.enable_latent_embedding = enable_latent_embedding
    ...
    def check_stats(self):
        stats_dict = {}
        keys = []
        traj_ids = self.trajectory_ids.tolist()
        for trajectory_id in tqdm(traj_ids, desc="Processing data", unit="item"):
            curr_traj_data = self.get_trajectory_data(trajectory_id)

            if trajectory_id == 0:
                keys = curr_traj_data.columns.tolist()
                for key in keys:
                    stats_dict[key] = []

            for key in curr_traj_data.columns:
                data_array: np.ndarray = np.stack(curr_traj_data[key])
                stats_dict[key].append(data_array)

        ep_stats = {}
        for key in keys:
            ep_ft_array = np.concatenate(stats_dict[key]) # ep_ft_array is already a np.ndarray
            axes_to_reduce = 0  # compute stats over the first axis
            keepdims = ep_ft_array.ndim == 1  # keep as np.array

            ep_stats[key] = self.get_feature_stats(ep_ft_array, axis=axes_to_reduce, keepdims=keepdims)

        print(f"[Dataset] Load {len(traj_ids)} trajectories from {self._dataset_name}")
    ...
    def _get_tasks(self) -> pd.DataFrame:
        ...
        try:
            ...
            print(f"process tasks error: {e}, read {tasks_path} instead")
            with open(tasks_path, "r") as f:
                tasks = [json.loads(line) for line in f]
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        data_item = self.transforms(self.get_step_data(trajectory_id, base_index))

        # --- DEBUG: 打印微调时 state 输入的类型 ---
        
        # print("\n--- [TRAINING DEBUG] Inspecting a sample's state dtype AFTER transforms ---")
        # for key, value in data_item.items():
        #     if "action" in key:
        #         value_type = type(value)
        #         dtype = getattr(value, 'dtype', 'N/A')
        #         print(f"    [DEBUG] Key: '{key}', Actual Type: {value_type}, Dtype: {dtype}")
        # print("--- [TRAINING DEBUG] This message will only be shown once. ---\n")
            
        # --- END DEBUG ---


        if self.enable_latent_embedding:
            image_inputs = data_item['eagle_content']['image_inputs']
            data_item['eagle_content']['future_image_inputs'] = image_inputs[1:]
            data_item['eagle_content']['image_inputs'] = image_inputs[0:1]
            data_item['eagle_content']['text_list'][0] = data_item['eagle_content']['text_list'][0].replace('<image-2>','')

        return data_item

    def debug_get_item(self, index: int):
        trajectory_id, base_index = self.all_steps[index]
        items = self.get_step_data(trajectory_id, base_index)
        return items
    ...
    def get_step_data(self, trajectory_id: int, base_index: int) -> dict:
        ...
        for modality in self.modality_keys:
            ...
            for key in self.modality_keys[modality]:
                if modality == 'video' and 'future' in key:
                    video_key = key.replace("_future","")
                    future_image = self.get_data_by_modality(trajectory_id, modality, video_key, base_index + 16)
                    data[key] = future_image
                else:
        ...
        #     # timestep t + 16
        #     modality = 'video'
        #     for video_key in self.modality_keys[modality]:
        #         # video_key = self.modality_keys[modality][0]
        #         future_image = self.get_data_by_modality(trajectory_id, modality, video_key, base_index + 16)
        #         data[video_key] = np.concatenate([
        #             data[video_key], # current image
        #             future_image     # future image: timestep + 16
        #         ], axis=0)

        # # --- DEBUG: Check for NaN values in the loaded raw data ---
        # for key, value in data.items():
        #     # We only check numpy arrays, as they contain the numerical data (state, action, etc.)
        #     if isinstance(value, np.ndarray):
        #         if np.isnan(value).any():
        #             print(
        #                 f"\n/!\\ [DATA DEBUG] NaN DETECTED in raw data! "
        #                 f"Key: '{key}', Trajectory ID: {trajectory_id}, Base Index: {base_index}\n"
        #             )
        # # --- END DEBUG ---
    ...
    def get_trajectory_data(self, trajectory_id: int) -> pd.DataFrame:
        ...
        if self.curr_traj_id == trajectory_id and self.curr_traj_data is not None:
            # 增加文本多样性
            if self.instr_use_episode_index:
                self.curr_traj_data.task_index = self.curr_traj_data.episode_index
            ...
            # 增加文本多样性
            curr_traj_data = pd.read_parquet(parquet_path)
            if self.instr_use_episode_index:
                curr_traj_data.task_index = curr_traj_data.episode_index
            return curr_traj_data
    ...
    def retrieve_data_and_pad(
        ...
        # --- DEBUG START ---
        if np.any(step_indices[~padding_positions] >= array.shape[0]):
            print("--- DEBUG: POTENTIAL INDEXERROR ---")
            print(f"array.shape: {array.shape}")
            print(f"max_length: {max_length}")
            print(f"step_indices: {step_indices}")
            print(f"padding_positions: {padding_positions}")
            print(f"indices_to_access: {step_indices[~padding_positions]}")
            print(f"max index to access: {np.max(step_indices[~padding_positions])}")
    ...
    def get_video(
        ...
        try:
            ...
            print(f"Error:{e} | get_video for video_path {video_path}, video_timestamp {video_timestamp}")
            return get_frames_by_timestamps(
                video_path.as_posix(),
                np.maximum(video_timestamp-1, 0),
                video_backend=self.video_backend,
                video_backend_kwargs=self.video_backend_kwargs,
            )
...
class LeRobotMixtureDataset(Dataset):
    ...
    def __init__(
        ...
        merge_metadata=True, # 预训练时, merge_metadata=False
        pretrain=False, # 是否开启预训练.
    ):
        """
        ...
        self.raw_sampling_weights = np.array(dataset_sampling_weights)
        ...
        print("##############################################################")
        print(f"# Raw Dataset Lengths: {self._dataset_lengths.sum()} samples")
        sampling_lengths = 0
        for idx, dataset_item in enumerate(data_mixture):
            dataset_name_ = dataset_item[0].dataset_name
            dataset_sampling_weight_ = self._dataset_sampling_weights[idx] * 100 # sw %
            dataset_len_ = len(dataset_item[0])
            dataset_name_ = dataset_name_.replace("_lerobot", "").replace("_converted_externally_to_rlds","")
            # sw: Sampling Weight, len: sub-dataset length, swlen: 采样数据集的长度
            sw = dataset_sampling_weight_
            swlen = dataset_len_*dataset_sampling_weights[idx]
            weight = dataset_sampling_weights[idx]
            print(f"## [{dataset_name_:<28}] len={dataset_len_:>10}, sw={sw:>6.2f}%, swlen={swlen:>8.0f}, weight={weight}")
            sampling_lengths += int(swlen)
            # print(f"## [{dataset_name_}]: \t sampling weight={dataset_sampling_weight_:.4f}, sub-dataset length={dataset_len_}")
        print(f"# Mixture Dataset Sampling Lengths: {sampling_lengths} samples")
        print("##############################################################")
        ...
        for dataset in self.datasets:
            ...
            # if self.balance_trajectory_weights=True: 每个轨迹的步长的采样率不一样,步长多的应该多采样,才能完整遍历到,每个步长被训练的频率均等.
            # if self.balance_trajectory_weights=False: 每个轨迹的采样率一致,那么短的轨迹和长的轨迹同时被采样到,在总的步长不变时,短的轨迹可能会被多采样,也就是不管轨迹长度,每种轨迹被训练的频率均等
        ...
        if not pretrain:
            print(f"############ merge multi-dataset metadata ##############")
            self.update_metadata(metadata_config)
        else:
            self.update_pretrain_metadata(metadata_config)
            print(f"############ !!! DONOT merge multi-dataset metadata ##############")

        if self.pretrain:
            self.current_epoch = -1
            self.total_samples = sampling_lengths
            self.precomputed_samples = self.prepare_precomputed_samples()
    ...
    def prepare_precomputed_samples(self):
        """预计算混合索引序列，支持权重倍数复制和子采样。"""
        # epoch-sensitive种子（若无hashlib，可用简单替换）
        try:
            from hashlib import sha256
            def safe_hash(tup):
                return int(sha256(str(tup).encode()).hexdigest(), 16) % (2**64)
        except ImportError:
            def safe_hash(tup):
                return (self.epoch * 1000000 + self.seed) % (2**64)  # 备用简单哈希

        seed = safe_hash((self.epoch, self.seed))
        rng = np.random.default_rng(seed)

        # 使用原始权重计算
        total_steps_per_ds = self.dataset_lengths

        precomputed = []
        for ds_idx, dataset in tqdm(enumerate(self.datasets), desc="Processing datasets", total=len(self.datasets)):
        # for ds_idx, dataset in enumerate(self.datasets):
            raw_weight = self.raw_sampling_weights[ds_idx]
            len_ds = total_steps_per_ds[ds_idx]
            samples_per_ds = round(len_ds * raw_weight)

            if samples_per_ds == 0:
                continue

            traj_lengths = np.array(dataset.trajectory_lengths)
            traj_cumsum = np.cumsum(traj_lengths)
            total_steps = traj_cumsum[-1]  # 应等于len_ds

            full_copies = samples_per_ds // total_steps
            remainder = samples_per_ds % total_steps

            # 完整拷贝部分：多次添加全索引列表（无重复）
            for _ in range(full_copies):
                flat_indices = np.arange(total_steps)
                for flat_idx in flat_indices:
                    traj_idx = np.searchsorted(traj_cumsum, flat_idx, side='right')
                    base_index = flat_idx - (traj_cumsum[traj_idx - 1] if traj_idx > 0 else 0)
                    traj_id = dataset.trajectory_ids[traj_idx]
                    precomputed.append((dataset, traj_id, base_index))

            # 余数部分：无放回加权采样
            if remainder > 0:
                if self.balance_trajectory_weights:
                    step_weights = np.ones(total_steps) / total_steps
                else:
                    step_weights = np.concatenate(
                        [np.full(length, 1.0 / (len(traj_lengths) * length)) for length in traj_lengths if length > 0]
                    )
                flat_indices = rng.choice(total_steps, size=remainder, replace=False, p=step_weights)
                for flat_idx in flat_indices:
                    traj_idx = np.searchsorted(traj_cumsum, flat_idx, side='right')
                    base_index = flat_idx - (traj_cumsum[traj_idx - 1] if traj_idx > 0 else 0)
                    traj_id = dataset.trajectory_ids[traj_idx]
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        """
        # 每个子数据集单独使用各自的metadata和transforms
        # if self.pretrain:
        #     dataset, trajectory_name, step = self.precomputed_samples[index]
        # else:
        ...
        while True:
            if self.pretrain:
                ...
                dataset, trajectory_name, step = self.sample_step(index)
            try:
                data_item = dataset.get_step_data(trajectory_name, step)
                data_item = dataset.transforms(data_item)
                break
            except Exception as e:
                # with open('bugs/pretrain_debug.txt', 'a') as f:
                #     f.write(f"{dataset.dataset_name}, {trajectory_name}, {step}, {e}\n")
                print(f"Error occurred while getting item {index}, dataset_name={dataset.dataset_name}, {trajectory_name}, step={step}, error={e}\n")
                index = np.random.randint(0, len(self) - 1)
                if index >= len(self):
                    raise IndexError("Failed to retrieve data item after multiple attempts.")
        return data_item
    ...
    def __len__(self) -> int:
        ...
        """
        # import ipdb; ipdb.set_trace() 
        # --no-balance-trajectory-weights  --no-balance-dataset-weights 
        # train dataloader length: 6680
        # train dataset length: 427500
        # 如果balance: train dataloader length: 3351 train dataset length: 214408
        if self.pretrain:
    ...
    def update_metadata(self, metadata_config: dict) -> None:
        ...
        # separate metadata and sampling_weights for each dataset
        all_metadatas: dict[str, list[DatasetMetadata]] = {} 
        all_weights: dict[str, list[float]] = {}

        for i, dataset in enumerate(self.datasets): # 针对不同构型的数据集类型，能够依次对应其metadata和sampling_weights
            tag = dataset.tag
            all_metadatas.setdefault(tag, []).append(dataset.metadata)
            all_weights.setdefault(tag, []).append(float(self.dataset_sampling_weights[i]))

        for tag, metadatas in all_metadatas.items():
            weights = all_weights[tag]
            assert len(weights) == len(metadatas), (tag, len(weights), len(metadatas))
            self.merged_metadata[tag] = self.merge_metadata(
                metadatas=metadatas,
    ...
    def update_pretrain_metadata(self, metadata_config: dict) -> None:
        ...
        for dataset in self.datasets:
            ...
            self.merged_metadata[dataset_name] = dataset.metadata

        # import ipdb;ipdb.set_trace()

class EpochAwareSubset(Subset):
    """在 Subset 上转发 set_epoch，保持与原始数据集的兼容性。"""
    def set_epoch(self, epoch: int):
