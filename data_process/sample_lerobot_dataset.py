...
def sample_datasets(
    ...
    for folder in source_folders:
        try:
            ...
            if not unique_available_indices:
                print("Warning: 没有可供采样的 episodes，采样结果为空。")
            elif len(unique_available_indices) >= sample_num:
                # 3a. 如果独立 episode 数量足够，进行无放回采样，保证唯一性
                print(f"独立 episodes 充足，进行无放回采样以获取 {sample_num} 个唯一数据。")
                current_folder_sample_indices = random.sample(unique_available_indices, k=sample_num)
            else:
                # 3b. 如果独立 episode 数量不足，进行有放回采样，以凑够数量
                print(f"Warning: 独立 episodes 数量 ({len(unique_available_indices)}) 不足 {sample_num}。")
                print(f"将进行有放回采样以确保收集到 {sample_num} 个数据（可能包含重复）。")
                current_folder_sample_indices = random.choices(unique_available_indices, k=sample_num)
...
if __name__ == "__main__":
    ...
    # Add arguments
    parser.add_argument("--sources", default="/vla/users/lijiayi/robocasa_datasets/gr1_unified.PosttrainPnPNovelFromTrayToTieredshelfSplitA_GR1ArmsAndWaistFourierHands_1000", required=False, help="List of source folder paths")
    parser.add_argument("--output", default="/vla/users/lijiayi/robocasa_datasets_sample/gr1_unified.PosttrainPnPNovelFromTrayToTieredshelfSplitA_GR1ArmsAndWaistFourierHands_sample_30" ,required=False, help="Output folder path")
    parser.add_argument("--sample_num", default=30, type=int, required=False, help="Sample ratio")
