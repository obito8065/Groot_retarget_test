...
if [ "$DEBUG" = "true" ]; then
    ...
    BATCH_SIZE=200  # 每个GPU的batch size
    WORKERS=8
fi
...
OUTPUT_DIR=/mnt/workspace/users/lijiayi/GR00T_QwenVLA/output_ckpt/n1.5_pretrain_egohuman_fourier_tunevl_v0.0
...
if [ "$NUM_GPUS" -gt 1 ] && [ "$DEBUG" = "false" ]; then
    ...
    python scripts/gr00t_finetune.py \
        ...
        --no-tune-llm \
        --update_action_head \
        --tune_visual 2>&1 | tee ${JOB_LOGDIR}/$(hostname).log
fi
