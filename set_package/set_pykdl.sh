...
if [ ! -f "${PYTHON_LIB}/libpython3.11.so" ]; then
    echo "错误：未找到libpython3.11.so，路径：${PYTHON_LIB}/libpython3.11.so"
    exit 1
else
    echo "✅ 验证通过：找到Python库文件 ${PYTHON_LIB}/libpython3.11.so"
fi
...
-DPYTHON_LIBRARY=${PYTHON_LIB}/libpython3.11.so \
-DBUILD_PYTHON=ON \
...
ln -sf ${PYKDL_SO} /opt/conda/lib/python3.11/site-packages/
echo "✅ PyKDL安装完成！已链接到Conda环境：${PYKDL_SO} -> /opt/conda/lib/python3.11/site-packages/"

# 验证安装
/opt/conda/bin/python -c "import PyKDL; print('✅ PyKDL导入成功！版本：', PyKDL.__version__)"