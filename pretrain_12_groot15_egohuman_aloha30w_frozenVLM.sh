...
if [ "$DEBUG" = "true" ]; then
    ...
    BATCH_SIZE=384  # 每个GPU的batch size
    WORKERS=8
fi


SAVE_STEPS=2000  # 20000
EPOCHS=20
...
dataset_list=(
    /mnt/workspace/datasets/egodex30w_datasets/egodex_merge30w_fourier
)
data_config=(
  "ego_dex_standard"
)
...
OUTPUT_DIR=/mnt/workspace/users/lijiayi/GR00T_QwenVLA/output_ckpt/n1.5_pretrain_egodex_fourier_frozenVLM_v1.0
...
# 多节点训练使用torchrun启动
if [ "$NUM_GPUS" -gt 1 ] && [ "$DEBUG" = "false" ]; then
    ...
    python scripts/gr00t_finetune.py \
        ...
        --tune-llm \
        --update_action_head \
        --no-tune_visual 2>&1 | tee ${JOB_LOGDIR}/$(hostname).log
fi
